...
class Store {
  ...
  async importPatientDataFull(
    data: Patient & { evaluations?: PatientEvaluation[] },
  ): Promise<void> {
    const { evaluations, ...patient } = data;

    // Legacy compat: sportsProfile nested inside clinical
    if (
      (patient.clinical as any).sportsProfile &&
      (!patient.sportsProfile || patient.sportsProfile.length === 0)
    ) {
    ...
    if (evaluations && Array.isArray(evaluations)) {
      for (const ev of evaluations) {
        await supabaseService.upsertEvaluation({
          ...ev,
          patientId: patient.id,
        });
      }
      const otherEvals = this.evaluations.filter(
        (e) => e.patientId !== patient.id,
      );
      this.evaluations = [
        ...otherEvals,
        ...evaluations.map((e) => ({ ...e, patientId: patient.id })),
      ];
      save(this.K.evaluations, this.evaluations);
    }
    ...
    for (const m of patient.measurements || []) {
      if (m.linkedEvaluationId) {
        await supabaseService.saveMeasurement(m.linkedEvaluationId, {
          ...m,
          patientId: patient.id,
        });
      }
    ...
    for (const b of patient.bioimpedancias || []) {
      if (b.evaluation_id) {
        await supabaseService.saveBioimpedancia(b.evaluation_id, {
          ...b,
          patientId: patient.id,
        });
      }
    ...
    for (const s of patient.somatotypes || []) {
      if (s.linkedEvaluationId) {
        await supabaseService.saveSomatotype(s.linkedEvaluationId, {
          ...s,
          patientId: patient.id,
        } as any);
      }
    ...
    for (const m of patient.menus || []) {
      if (m.linkedEvaluationId) {
        await supabaseService.saveMenu(m.linkedEvaluationId, {
          ...m,
          patientId: patient.id,
        });
      }
    ...
    for (const f of patient.labs || []) {
      await supabaseService.upsertPatientFileMeta(
        { ...f, folder: "labs" },
        patient.id,
      );
    }
  ...
  importPatientData(
    ...
    if (evaluations && Array.isArray(evaluations)) {
      const otherEvaluations = this.evaluations.filter(
        (e) => e.patientId !== patient.id,
      );
  ...
  async saveDietaryEvaluation(
    ...
    const patient = this.patients.find(
      (p) =>
        p.dietaryEvaluations.some((d) => d.id === dietary.id) ||
        p.id === this.getEvaluationById(evaluationId)?.patientId,
    );
    if (patient) {
      const exists = patient.dietaryEvaluations.some(
        (d) => d.id === dietary.id,
      );
      const updatedEvaluations = exists
        ? patient.dietaryEvaluations.map((d) =>
            d.id === dietary.id ? dietary : d,
          )
        : [dietary, ...patient.dietaryEvaluations];

      const updatedPatient = {
        ...patient,
        dietaryEvaluations: updatedEvaluations,
      };
  ...
  async deleteDietaryEvaluation(id: string): Promise<void> {
    ...
    const patient = this.patients.find((p) =>
      p.dietaryEvaluations.some((d) => d.id === id),
    );
    if (patient) {
      const updatedEvaluations = patient.dietaryEvaluations.filter(
        (d) => d.id !== id,
      );
      const updatedPatient = {
        ...patient,
  ...
  async deleteMeasurement(id: string): Promise<void> {
    ...
    // Update local state
    const patient = this.patients.find((p) =>
      p.measurements.some((m) => m.id === id),
    );
    if (patient) {
      const updatedMeasurements = patient.measurements.filter(
        (m) => m.id !== id,
      );
      const updatedPatient = { ...patient, measurements: updatedMeasurements };
      this.patients = this.patients.map((p) =>
  ...
  async saveSomatotype(
    evaluationId: string,
    record: SomatotypeRecord,
  ): Promise<SomatotypeRecord> {
    ...
    const patient = this.patients.find((p) => p.id === evalObj.patientId);
    if (patient) {
      const exists = patient.somatotypes.some(
        (s) => s.id === record.id || s.id === saved.id,
      );
      const updatedSomatotypes = exists
        ? patient.somatotypes.map((s) =>
            s.id === record.id || s.id === saved.id ? savedRecord : s,
          )
  ...
  async saveBioimpedancia(evaluationId: string, record: any): Promise<any> {
    ...
    const patient = this.patients.find((p) => p.id === evalObj.patientId);
    if (patient) {
      const exists = patient.bioimpedancias.some(
        (b) => b.id === saved.id || b.id === record.id,
      );
      const updatedBioimpedancias = exists
        ? patient.bioimpedancias.map((b) =>
            b.id === saved.id || b.id === record.id ? saved : b,
          )
        : [saved, ...patient.bioimpedancias];

      const updatedPatient = {
        ...patient,
        bioimpedancias: updatedBioimpedancias,
      };
      this.patients = this.patients.map((p) =>
        p.id === updatedPatient.id ? updatedPatient : p,
      );
  ...
  async deleteBioimpedancia(id: string): Promise<void> {
    ...
    // Update local state
    const patient = this.patients.find((p) =>
      p.bioimpedancias.some((b) => b.id === id),
    );
    if (patient) {
      const updatedBioimpedancias = patient.bioimpedancias.filter(
        (b) => b.id !== id,
      );
      const updatedPatient = {
        ...patient,
  ...
  getMenuReferences(): MenuReferenceRecord[] {
    return this.menuReferences;
  }

  async saveMenuReference(
    ref: Partial<MenuReferenceRecord>,
  ): Promise<MenuReferenceRecord> {
    ...
    if (existing) {
      this.menuReferences = this.menuReferences.map((r) =>
  ...
  getMenuRecommendations() {
    return this.menuRecommendations;
  }

  async saveMenuRecommendation(
    rec: Partial<MenuRecommendationRecord>,
  ): Promise<MenuRecommendationRecord> {
    const saved = await supabaseService.saveMenuRecommendation({
      ...rec,
      ownerId: this.uid,
    });
    const existing = this.menuRecommendations.find((r) => r.id === saved.id);
    if (existing) {
      this.menuRecommendations = this.menuRecommendations.map((r) =>
        r.id === saved.id ? saved : r,
      );
    } else {
  ...
  async deleteMenuRecommendation(id: string): Promise<void> {
    ...
    this.menuRecommendations = this.menuRecommendations.filter(
      (r) => r.id !== id,
    );
  }
  ...
  async addAppointment(
    ...
    if (ownerId && ownerId !== "guest" && googleCalendarService.isConnected()) {
      googleCalendarService
        .createEvent(newAppt, ownerId)
        .then((googleEventId) => {
          if (!googleEventId) return;
          supabaseService
            .updateAppointmentGoogleEventId(newAppt.id, googleEventId)
            .catch(() => {});
          newAppt.googleEventId = googleEventId;
          this.appointments = this.appointments.map((a) =>
            a.id === newAppt.id ? { ...a, googleEventId } : a,
          );
          save(this.K.appointments, this.appointments);
        })
        .catch(() => {});
  ...
  async deleteAppointment(id: string): Promise<void> {
    ...
    if (
      ownerId &&
      ownerId !== "guest" &&
      apptToDelete?.googleEventId &&
      googleCalendarService.isConnected()
    ) {
      googleCalendarService
        .deleteEvent(apptToDelete.googleEventId, ownerId)
        .catch(() => {});
    }
  }

  async addAppointmentForNutritionist(
    ...
    googleCalendarService
      .createEvent(newAppt, nutritionistId)
      .then((googleEventId) => {
        if (!googleEventId) return;
        supabaseService
          .updateAppointmentGoogleEventId(newAppt.id, googleEventId)
  ...
  async addEvaluation(
    ...
    const savedEv = await supabaseService.createEvaluation(
      patientId,
      date,
      title,
    );
  ...
  async updateEvaluation(
    evaluationId: string,
    patch: Partial<Omit<PatientEvaluation, "id" | "patientId">>,
  ...
  async getAppointmentsForNutritionist(
    nutritionistId: string,
  ): Promise<Appointment[]> {
  ...
  async updateAppointmentForNutritionist(
    nutritionistId: string,
  ...
  async deleteAppointmentForNutritionist(
    nutritionistId: string,
