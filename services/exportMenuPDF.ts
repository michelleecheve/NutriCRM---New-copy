...
export async function exportMenuPDFAsBlob(menu: GeneratedMenu): Promise<Blob> {
  if (!menu.menuData) throw new Error('exportMenuPDFAsBlob: menu has no menuData');
  ...
  try {
    ...
    return await worker.get('pdf').then((pdf: any) => pdf.output('blob') as Blob);
  } finally {
    root.unmount();
    document.body.removeChild(container);
  }
