...
import { StatusFilter, APPOINTMENT_STATUSES } from "./StatusFilter";
...
export const CalendarHistorialTable: React.FC<CalendarHistorialTableProps> = ({
  ...
  const toggleCollapsed = () => {
    setIsCollapsed((v) => {
      const next = !v;
      try {
        localStorage.setItem(HISTORIAL_OPEN_KEY, String(!next));
      } catch {}
      return next;
    });
  };
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [activePreset, customFrom, customTo, statusFilter]);
    ...
    if (deletingAppt) {
      ...
      try {
        if (isManagingForOtherNutritionist && targetNutritionistId) {
          await store.deleteAppointmentForNutritionist(
            targetNutritionistId,
            deletingAppt.id,
          );
        } else {
  ...
  const historyAppointments = [...appointments]
    .filter((appt) => {
      ...
      if (!statusFilter.includes(appt.status)) return false;
      const range =
        activePreset === "custom"
          ? customFrom && customTo
    ...
    .sort((a: any, b: any) => {
      const dateA = new Date(
        a.updatedAt || a.createdAt || a.date + "T00:00:00",
      );
      const dateB = new Date(
        b.updatedAt || b.createdAt || b.date + "T00:00:00",
      );
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(historyAppointments.length / ITEMS_PER_PAGE),
  );
  ...
  const handleExportCSV = (scope: "filtered" | "all") => {
    ...
    const rows = dataToExport.map((appt) => {
      ...
      return [
        (appt as any).updatedAt
          ? new Date((appt as any).updatedAt).toLocaleString("es-ES")
          : "N/A",
        appt.status,
        appt.patientName,
        `${d}/${m}/${y}`,
  ...
  return (
    ...
    >
      <div
        className={`p-6 ${!isCollapsed ? "border-b border-slate-100" : ""} flex items-center justify-between gap-3 cursor-pointer select-none`}
        onClick={(e) => {
          e.stopPropagation();
          toggleCollapsed();
        }}
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <div className="flex-1 sm:flex-none">
            <h3 className="font-bold text-slate-900 text-lg">
              Historial de Citas en el Calendario
            </h3>
            <p className="text-xs text-slate-400">
              Citas registradas hasta hoy, ordenadas por última actividad. No
              incluye citas que aún no han pasado.
            </p>
          </div>
        </div>
        <ChevronDown
          className={`w-5 h-5 text-slate-400 transition-transform flex-shrink-0 ${!isCollapsed ? "rotate-180" : ""}`}
        />
      </div>

      {!isCollapsed && (
        <div
          className="px-6 py-4 border-b border-slate-100 flex flex-wrap items-center gap-3"
        ...
        >
          <div onClick={(e) => e.stopPropagation()}>
            <StatusFilter
              ...
              label="Filtro Status"
              buttonClassName="flex items-center gap-2 px-4 py-2.5 rounded-lg border text-sm font-semibold transition-all bg-white border-slate-200 text-slate-600 hover:bg-slate-50 shadow-sm"
            />
          </div>
          <DateFilter
            ...
            onPresetChange={(p) => {
              setActivePreset(p);
              if (p !== "custom") setIsFilterOpen(false);
            }}
          ...
          <table className="w-full text-left text-sm relative">
            <thead className="bg-slate-50 text-slate-500 font-bold text-xs uppercase">
              <tr>
                ...
                <th className="px-6 py-4 bg-slate-50 min-w-[140px]">
                  Fecha Cita
                </th>
                ...
                <th className="px-6 py-4 text-center bg-slate-50">Acción</th>
              </tr>
            </thead>
              ...
              {paginatedAppointments.map((appt, idx) => (
                ...
                >
                  <td className="px-6 py-4">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-bold border ${getStatusStyles(appt.status)}`}
                    >
                      {appt.status}
                    </span>
                  </td>
                    ...
                    <div className="flex items-center justify-end gap-1">
                      {appt.status !== "Completada" && appt.status !== "Cancelada" && (
                        <button
                          ...
                          className="text-emerald-600 hover:text-white hover:bg-emerald-700 disabled:opacity-70 disabled:cursor-wait px-2 py-1.5 rounded-lg transition-colors text-xs font-bold leading-tight text-center min-w-[84px] flex items-center justify-center"
                        >
                      ...
                      <button
                        ...
                        className="text-slate-400 hover:text-white p-2 hover:bg-emerald-700 rounded-lg transition-colors"
                        title="Editar"
                      >
                      ...
                      <button
                        ...
                        className="text-slate-400 hover:text-white p-2 hover:bg-red-700 rounded-lg transition-colors"
                        title="Eliminar"
                      >
      ...
      {!isCollapsed && historyAppointments.length > 0 && (
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
          <span className="text-sm text-slate-400 font-medium">
            {historyAppointments.length} registro
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 border border-slate-100">
            <div className="p-6 text-center">
              ...
              <p className="text-slate-500 text-sm mb-6">
                ¿Seguro que quiere eliminar este registro de cita de{" "}
                <span className="font-bold text-slate-700">
                  {deletingAppt.patientName}
                </span>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={isDeleting}
                  className="flex-1 py-3 bg-slate-100 text-slate-600 rounded-xl font-bold hover:bg-slate-200 disabled:opacity-50 transition-colors"
                >
                  Cancelar
                </button>
                <button
                  ...
                  disabled={isDeleting}
                  className="flex-1 py-3 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 disabled:opacity-70 disabled:cursor-wait shadow-lg shadow-red-600/20 transition-colors flex items-center justify-center gap-2"
                >
                  {isDeleting ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
