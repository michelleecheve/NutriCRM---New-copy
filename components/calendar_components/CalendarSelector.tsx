...
export const CalendarSelector: React.FC<CalendarSelectorProps> = ({
  ...
  const isMyCalendar = selectedNutritionistId === currentUserId;
  const selectedNutri = linkedNutritionists.find(n => n.id === selectedNutritionistId);

  const displayName = isMyCalendar
    ? 'Mi Calendario'
    : selectedNutri
      ? `${selectedNutri.profile.professionalTitle || ''} ${selectedNutri.profile.name}`.trim()
      : 'Mi Calendario';
          ...
          <>
            ...
            <div className="absolute right-0 top-[calc(100%+6px)] z-20 bg-white border border-slate-200 rounded-2xl shadow-xl min-w-[220px] overflow-hidden">
              ...
              {linkedNutritionists.length > 0 && (
                <>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-4 pt-2 pb-1 border-t border-slate-100">
                  ...
                  {linkedNutritionists.map(nutri => {
                    ...
                    return (
                      <button
                        key={nutri.id}
                        onClick={() => { onNutritionistChange(nutri.id); setOpen(false); }}
