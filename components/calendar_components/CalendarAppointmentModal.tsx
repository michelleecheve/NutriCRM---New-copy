...
const inputCls =
  "w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 text-slate-900 focus:bg-white focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all text-sm";

// ─── Component ────────────────────────────────────────────────────────────────
export const CalendarAppointmentModal: React.FC<
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isManagingForOtherNutritionist && targetNutritionistId) {
        if (isEdit) {
          await store.updateAppointmentForNutritionist(targetNutritionistId, {
            ...(appointmentData as Appointment),
          } as any);
        } else {
          await store.addAppointmentForNutritionist(targetNutritionistId, {
      ...
      <div className="bg-white rounded-2xl w-full max-w-3xl shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-300 border border-slate-100 my-auto">
        ...
        <div className="px-8 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div className="flex items-center gap-3">
            <div
              className={`p-2 rounded-lg ${isEdit ? "bg-blue-100 text-blue-600" : "bg-emerald-100 text-emerald-600"}`}
            >
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                {isEdit
                  ? "Modifica los detalles o el estado"
                  : "Complete los detalles de la sesión"}
              </p>
            </div>
          </div>
          <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-100">
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    placeholder="Notas, motivo de consulta, indicaciones..."
                    value={tempNotes}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(["Presencial", "Video"] as const).map((mod) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setFormData({ ...formData, modality: mod })
                        }
