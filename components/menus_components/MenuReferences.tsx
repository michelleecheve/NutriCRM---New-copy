        ...
        <thead>
          <tr className="bg-slate-50">
            ...
            {PORTION_GROUPS.map((g) => (
              <th
                key={g.key}
                className="px-3 py-3 text-center text-xs font-bold text-slate-500 uppercase tracking-wider"
              >
                {g.label}
              </th>
            ))}
            {!readOnly && <th className="px-2 py-3 w-8" />}
          </tr>
        </thead>
        <tbody>
          {meals.map((slot, i) => (
            ...
            >
              <td className="px-3 py-2">
                {readOnly ? (
                  <span
                    className={`inline-flex items-center px-2.5 py-1 rounded-lg border text-xs font-bold ${LABEL_COLORS[slot.label]}`}
                  >
                    {slot.label}
                  </span>
                ) : (
                  <select
                    value={slot.label}
                    onChange={(e) =>
                      onChangeLabel(slot.id, e.target.value as MealLabel)
                    }
          ...
          <tr className="bg-emerald-50 border-t-2 border-emerald-200">
            ...
            {PORTION_GROUPS.map((g) => (
              <td
                key={g.key}
                className="px-3 py-2 text-center font-extrabold text-emerald-700"
              >
                {totals[g.key]}
              </td>
            ))}
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={() =>
            setActiveTab(
              ALL_TABS[Math.min(ALL_TABS.length - 1, currentIndex + 1)],
            )
          }
              ...
              <div key={slot.id} className="space-y-1.5">
                ...
                <textarea
                  value={
                    (
                      weeklyMenu[
                        activeTab as WeekDayKey | "domingoV2"
...
function parseYamlToReferenceData(yaml: string): MenuReferenceData {
  ...
  const porcSection =
    yaml.match(
      /PORCIONES_POR_TIEMPO:([\s\S]*?)(?=\nWEEKLY_MENU:|\nNOTAS:)/,
    )?.[1] ?? "";
  const meals: MealSlot[] = Object.entries(SLOT_MAP).map(
    ([yamlKey, { id, label }]) => {
      ...
      return {
        ...
        portions: {
          lacteos: num("lacteos"),
          vegetales: num("vegetales"),
          frutas: num("frutas"),
          cereales: num("cereales"),
          carnes: num("carnes"),
          grasas: num("grasas"),
        },
      };
  ...
  const weeklySection =
    yaml.match(/WEEKLY_MENU:([\s\S]*?)(?=\nNOTAS:|\s*$)/)?.[1] ?? "";
  const weekDays: (WeekDayKey | "domingoV2")[] = [
    "lunes",
    "martes",
    "miercoles",
    "jueves",
...
const ExchangeRefEditor: React.FC<{
  ...
  const [meals, setMeals] = useState<ExchangeMeal[]>(
    initialData.exchangeMenu.meals || [],
  );
  ...
  const [hydration, setHydration] = useState(
    initialData.exchangeMenu.hydration || "",
  );
  ...
  const handleChangePortionLabel = (slotId: string, label: MealLabel) => {
    setPortions((prev) =>
      prev.map((s) => (s.id === slotId ? { ...s, label } : s)),
    );
  };

  const handleChangePortions = (
    slotId: string,
    group: keyof MealPortions,
    value: number,
  ) => {
    setPortions((prev) =>
      prev.map((s) =>
        s.id === slotId
          ? { ...s, portions: { ...s.portions, [group]: value } }
          : s,
      ),
    );
  };
    ...
    setMeals((prev) =>
      prev.map((m) =>
        m.id === mealId
          ? {
              ...m,
              examples: m.examples.map((e, i) => (i === colIdx ? value : e)),
            }
          : m,
      ),
    );
  ...
  const addMeal = () => {
    ...
    setMeals((prev) => [
      ...prev,
      { id, label: "Nuevo tiempo", examples: Array(numCols).fill("") },
    ]);
  };
  ...
  const addColumn = () => {
    ...
    setMeals((prev) =>
      prev.map((m) => ({ ...m, examples: [...m.examples, ""] })),
    );
  };
  ...
  const removeColumn = (colIdx: number) => {
    ...
    setMeals((prev) =>
      prev.map((m) => ({
        ...m,
        examples: m.examples.filter((_, i) => i !== colIdx),
      })),
    );
  };
  ...
  const handleSave = async () => {
    setSaveError("");
    if (!name.trim()) {
      setSaveError("Ingresa un nombre para la plantilla.");
      return;
    }
    if (meals.length === 0) {
      setSaveError(
        "Agrega al menos un tiempo de comida en la tabla de intercambio.",
      );
      return;
    }
    setIsSaving(true);
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {readOnly && (
          <span className="text-xs font-bold text-slate-400 uppercase bg-slate-100 px-3 py-1.5 rounded-lg">
            Solo lectura
          </span>
        )}
      </div>
            ...
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                {columnLabels.map((label, i) => (
                  <th
                    key={i}
                    className="px-2 py-3 text-[10px] font-black text-slate-600 uppercase"
                    style={{ minWidth: 180 }}
                  >
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-sky-500 uppercase">
              Meta Hidratación
            </label>
            <textarea
              value={hydration}
...
export const MenuReferences: React.FC<{
  ...
  const [formData, setFormData] =
    useState<MenuReferenceData>(emptyReferenceData());
  const [exchangeFormData, setExchangeFormData] =
    useState<ExchangeReferenceData>(emptyExchangeReferenceData());
  ...
  const [isImportExchangeFromMenuOpen, setIsImportExchangeFromMenuOpen] =
    useState(false);
  const [page, setPage] = useState(1);
  const PAGE_SIZE = 10;

  const isExchange = (r: MenuReferenceRecord) =>
    (r.data as any)?.type === "INTERCAMBIO";
  ...
  const handleAddAsReference = async (
    ...
    const mealsOrder: string[] =
      lunesData?.mealsOrder ||
      Object.keys((plan.portions as any)?.byMeal || {});

    const meals: MealSlot[] = mealsOrder.map((slotId: string) => {
      const mealInfo = lunesData?.[slotId];
      const rawLabel = mealInfo?.label || slotId;
      const portions =
        (plan.portions as any)?.byMeal?.[slotId] || emptyMealPortions();
    ...
    refWeeklyMenu.domingo = {
      note: (plan.weeklyMenu as any)?.domingo?.note || "",
    };
    ...
    const refData: MenuReferenceData = {
      ...
      hydration:
        (plan.weeklyMenu as any)?.domingo?.hydration || "2.5L Agua/Día",
  ...
  const handleAddExchangeAsReference = async (
    menu: GeneratedMenu,
    patient: Patient,
  ): Promise<void> => {
    const plan = menu.menuData as MenuPlanData;
    ...
    const refData: ExchangeReferenceData = {
      ...
      name:
        menu.name ||
        (patientName ? `Intercambio - ${patientName}` : "Intercambio"),
  ...
  const sortedItems = [...items]
    .filter((r) =>
      activeRefTab === "INTERCAMBIO" ? isExchange(r) : !isExchange(r),
    )
    .sort((a, b) => {
      if ((a.data as any).kcal !== (b.data as any).kcal)
        return (a.data as any).kcal - (b.data as any).kcal;
  ...
  const openView = (record: MenuReferenceRecord) => {
    ...
    if (isExchange(record)) {
      ...
      try {
        setPreviewPlan(
          MenuReferenceDataToMenuPlanData(record.data as MenuReferenceData),
        );
      } catch {
        setPreviewPlan(null);
      }
  ...
  const handleChangeMealText = (
    ...
    setFormData((prev) => ({
      ...
      weeklyMenu: {
        ...prev.weeklyMenu,
        [day]: {
          ...(prev.weeklyMenu[day] as Record<string, string>),
          [slotId]: value,
        },
      },
    }));
  ...
  const handlePreview = () => {
    try {
      setPreviewPlan(MenuReferenceDataToMenuPlanData(formData));
      setSaveError("");
    } catch (e: any) {
      setSaveError(e?.message || "Error al generar preview.");
    }
  };

  const handleSave = async () => {
    setSaveError("");
    if (!formData.kcal || formData.kcal < 800) {
      setSaveError("Las kcal deben ser mayores a 800.");
      return;
    }
    if (formData.meals.length === 0) {
      setSaveError("Agrega al menos un tiempo de comida.");
      return;
    }
    try {
      await store.saveMenuReference({
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-900">
                    Agregar Plantilla desde Historial de Intercambio
                  </h3>
                  <p className="text-sm text-slate-500">
              ...
              <button
                onClick={() => setIsImportExchangeFromMenuOpen(false)}
                className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-xl transition-colors"
              >
                <X className="w-6 h-6" />
              </button>
            ...
            <div className="flex-1 overflow-y-auto">
              <MenuHistory
                onAddAsReference={handleAddExchangeAsReference}
                filterType="intercambio"
              />
            </div>
          </div>
        </div>
        ...
        <div className="p-6 border-b border-slate-100">
          <div className="flex items-start justify-between gap-4 mb-4">
            ...
            {mode === "LIST" ? (
              ...
              <button
                onClick={backToList}
                className="flex items-center gap-2 border border-slate-200 hover:bg-slate-50 text-slate-700 font-semibold px-4 py-2 rounded-xl text-sm transition-colors"
              >
                ← Volver
              </button>
            )}
        ...
        <div className="p-6">
          {sortedItems.length === 0 ? (
            <div className="p-12 text-center">
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${activeRefTab === "INTERCAMBIO" ? "bg-indigo-50" : "bg-blue-50"}`}
              >
                <FileText
                  className={`w-8 h-8 ${activeRefTab === "INTERCAMBIO" ? "text-indigo-400" : "text-blue-400"}`}
                />
              </div>
            ...
            <div className="space-y-3">
              <div className="overflow-x-auto rounded-xl border border-slate-200">
                <table className="w-full text-sm border-collapse">
                  ...
                  <tbody>
                    {pageItems.map((r, i) => {
                      const d = r.data as MenuReferenceData &
                        ExchangeReferenceData;
                      const displayDate = d.evaluationDate
                        ? new Date(
                            d.evaluationDate + "T00:00:00",
                          ).toLocaleDateString()
        ...
        <div className="p-6 space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              {!isReadOnly && (
                <span className="text-[10px] text-slate-400 font-medium">
                  Usa el dropdown de cada fila para cambiar la etiqueta del
                  tiempo
                </span>
              )}
            </div>
          ...
          <div className="space-y-4">
            ...
            <WeeklyMenuEditor
              ...
              onHydrationChange={(v) =>
                setFormData((prev) => ({ ...prev, hydration: v }))
              }
            />
          </div>
