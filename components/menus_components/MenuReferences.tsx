...
}> = ({ notes, readOnly, onChange }) => (
  <div className="space-y-3">
    {notes.map((note, i) => (
      <div key={i} className="flex gap-2 items-start">
        <span className="mt-2.5 text-slate-400 text-xs font-bold">•</span>
        <input
          type="text"
          value={note}
          onChange={e => { const n = [...notes]; n[i] = e.target.value; onChange(n); }}
          readOnly={readOnly}
          className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-medium text-slate-700 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all"
          placeholder="Nota o recomendación..."
        />
        {!readOnly && (
          <button onClick={() => onChange(notes.filter((_, idx) => idx !== i))} className="mt-1.5 p-1.5 text-slate-400 hover:text-red-500 transition-colors">
            <X className="w-4 h-4" />
          </button>
        )}
      </div>
...
function parseYamlToReferenceData(yaml: string): MenuReferenceData {
  ...
  // Notes
  const notasSection = yaml.match(/^NOTAS:\n((?:- [^\n]*\n?)*)/m)?.[1] ?? '';
  const notes = notasSection.split('\n')
    .filter(l => l.startsWith('- '))
    .map(l => l.replace(/^- /, '').trim());

  return { kcal, type, meals, weeklyMenu, hydration, notes };
}
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      {mode === "LIST" ? (
        ...
        <div className="p-6 space-y-8">
          ...
          {/* 4. Notas */}
          <div className="space-y-4">
            <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest">4. Notas y Recomendaciones</h4>
            <NotesEditor
