...
1. TRANSCRIBE todo el texto tal como aparece en el documento
2. IDENTIFICA los días de la semana presentes: lunes, martes, miercoles, jueves, viernes, sabado, domingo
3. IDENTIFICA los tiempos de comida de cada día
4. MAPEA cada tiempo encontrado al ID configurado correspondiente usando las reglas de abajo
5. GENERA el JSON con el contenido copiado exactamente

IDs CONFIGURADOS (en orden del día, de arriba a abajo):
${mealList}
...
- Responde ÚNICAMENTE con el JSON puro, sin markdown, sin explicaciones

ESTRUCTURA DE RESPUESTA — incluye TODOS estos IDs en "grid": ${mealIdList}
{
  "found_days": ["lunes", "martes", "miercoles", "jueves", "viernes", "sabado"],
  "missing_days": ["domingo"],
  "found_meal_count": ${meals.length},
  "has_notes": false,
  "has_hydration": false,
  "grid": {
${gridTemplate}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {/* ERROR */}
              {!aiImportLoading && aiImportError && !aiImportResult && (
                  ...
                  <div className="px-5 pt-3 pb-2">
                    ...
                    {aiImportFile ? (
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-slate-200 rounded-xl px-3 py-2.5 text-xs font-bold text-slate-400 hover:border-violet-300 hover:text-violet-500 hover:bg-violet-50/50 transition-all"
                      >
                        <FileText className="w-3.5 h-3.5" />
                        Subir PDF o imagen (opcional)
                      </button>
                    )}
                  </div>
                </>
              ...
              {!aiImportLoading && aiImportResult && (
                <div className="px-5 pt-4 pb-2 space-y-3">
                  ...
                  {/* Tiempos de comida + contenido detectado */}
                    ...
                    {(() => {
                      ...
                      return withContent < meals.length ? (
                        ...
                        <p className="text-[10px] text-emerald-600 flex items-center gap-1">
                          ...
                          Todos los tiempos tienen contenido detectado
                        </p>
                      );
                    })()}
                  </div>
            ...
            <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-slate-100 shrink-0">
              ...
              {!aiImportLoading && aiImportResult && !aiImportApplied && (
                <button
                  onClick={applyAiResult}
                  className="flex items-center gap-1.5 px-5 py-2 rounded-xl text-xs font-bold bg-emerald-600 text-white hover:bg-emerald-700 shadow-sm transition-all"
                >
