    ...
    <table style={headerStyle}>
      <tbody>
        <tr>
          ...
          {/* Right: plan title */}
          <td
            style={{ verticalAlign: "middle", textAlign: "right", padding: 0 }}
          >
            {titleParts[0] && (
              <div
                style={{
...
// ── Footer ─────────────────────────────────────────────────────────────────────
const Footer: React.FC<{ nutritionist: MenuPlanData["nutritionist"] }> = ({
  nutritionist,
}) => {
  ...
  if (show("showPersonalPhone") && nutritionist.personalPhone)
    contactItems.push({
      label: "Tel. Personal",
      value: nutritionist.personalPhone,
    });
      ...
      {(nameLeft.length > 0 || showWebsite) && (
        <div style={{ fontSize: "8px", color: "#1e293b", marginBottom: "2px" }}>
          ...
          {showWebsite && (
            <>
              <span
                style={{ fontWeight: 800, color: "#1e293b", margin: "0 6px" }}
              >
                |
              </span>
              <span
            ...
            <tr>
              {contactItems.map((item, i) => (
                <td
                  ...
                  style={{
                    paddingRight:
                      i < contactItems.length - 1 ? "14px" : undefined,
                    verticalAlign: "middle",
                    whiteSpace: "nowrap",
                  }}
                >
...
// ── SplitCell: dos cards separadas (nota + hidratación) para layout 4col ───────
const SplitCell: React.FC<{
  ...
  return (
    <div style={{ flex, minWidth: 0, display: "flex", flexDirection: "column", gap: "6px" }}>
      ...
      {/* Card de hidratación — fondo blanco, label gris pequeño */}
      <div style={{ border: "1px solid #e2e8f0", borderRadius: ts.cardRadius, overflow: "hidden" }}>
        ...
        {gridLayout === "4col" ? (
          <>
            ...
            <div style={{ display: "flex", gap: "6px", marginBottom: "6px" }}>
              {(["lunes", "martes", "miercoles", "jueves"] as WeekDayKey[]).map(
                (day) => (
                  <DayCard
                    key={day}
                    label={WEEKDAY_LABELS[day]}
                    day={data.weeklyMenu[day]}
                  />
                ),
              )}
            </div>
            {/* Fila 2: Vie – Sab + SplitCell (doble ancho = Dom libre + Hidratación) */}
            <div style={{ display: "flex", gap: "6px", marginBottom: "8px" }}>
              <DayCard
                label={WEEKDAY_LABELS["viernes"]}
                day={data.weeklyMenu["viernes"]}
              />
              <DayCard
                label={WEEKDAY_LABELS["sabado"]}
                day={data.weeklyMenu["sabado"]}
              />
              {/* Cols 3+4: contenedor que iguala alturas entre sí sin estirarse a la fila */}
      ...
      >
        ...
        {gridLayout === "4col" ? (
          <>
            ...
            {/* Fila 2: Vie – Sab – Dom + SplitCell (notas + hidratación) */}
            <div style={{ display: "flex", gap: "6px", marginBottom: "8px" }}>
              <DayCard
                label={WEEKDAY_LABELS["viernes"]}
                day={data.weeklyMenu["viernes"]}
