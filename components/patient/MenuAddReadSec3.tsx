...
export const MenuAddReadSec3: React.FC<MenuAddReadSec3Props> = ({
  ...
  const handleGenerateAi = async () => {
    ...
    try {
      ...
      // Also generate a draft text representation for the editor if needed
      // For now, we'll just use the structured data directly
  ...
  // ─── Preview Logic ─────────────────────────────────────────────────────────
  // Removed as structured data is now used directly

  // ─── PDF Export ────────────────────────────────────────────────────────────
  // Logic moved to MenuExportPDF component
  ...
  const PortionsEditor = () => {
    ...
    const [meals, setMeals] = useState<{id: string, label: string}[]>(() => {
      // Get order from Monday's mealsOrder or default
    ...
    const handleSave = () => {
      ...
      // Also update weeklyMenu order and labels to match
  ...
  const DayEditor = ({ dayKey }: { dayKey: string }) => {
    ...
    // Internal state for dynamic meals (only for non-domingo)
        ...
        <div className="p-8 space-y-8 animate-in slide-in-from-top-2 duration-300">
          ...
          <div className="flex flex-wrap gap-4">
            <button 
              ...
              className={`flex-1 min-w-[200px] flex items-center justify-center gap-2 py-3 rounded-2xl font-bold transition-all shadow-lg ${
                isGenerating 
            ...
            {/* Removed Preview Menu button as structured data is used directly */}

            <MenuExportPDF 
              elementId="menu-print-area"
              filename={`Menu_${patient.firstName}_${new Date().toISOString().split('T')[0]}`}
              disabled={!menuPreviewData}
              className={`flex-1 min-w-[200px] flex items-center justify-center gap-2 py-3 rounded-2xl font-bold transition-all border-2 ${
                !menuPreviewData 
