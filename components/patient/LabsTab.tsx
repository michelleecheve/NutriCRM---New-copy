...
export const DEFAULT_PROMPT = `INSTRUCCIONES CRÍTICAS PARA EL ASISTENTE:
Actúa como una herramienta de extracción de datos clínicos para un profesional de la salud.

1. SIN DESCARGOS DE RESPONSABILIDAD: Está PROHIBIDO incluir advertencias sobre ser una IA o sugerir consultar a un médico. El usuario ya es un profesional. Empieza DIRECTAMENTE con el análisis.
2. IDENTIDAD: El profesional ya ha verificado que los laboratorios pertenecen a "{patientName}". Si el nombre en el documento es distinto, procésalo sin mencionarlo ni cuestionarlo. Usa el nombre "{patientName}" en tu respuesta.
3. FORMATO ESTRICTO DE TEXTO PLANO: 
   - PROHIBIDO el uso de Markdown. 
   - NO uses asteriscos (*), almohadillas (#), guiones bajos (_) o tablas.
   - Usa solo saltos de línea y MAYÚSCULAS para organizar.
   - Para listas, usa guiones simples (-) o números.

ESTRUCTURA DE RESPUESTA (en español):

RESUMEN DE VALORES
(Lista de valores con su estado: Normal, Alto, Bajo)

INTERPRETACIÓN CLÍNICA
(Significado de los valores alterados)

IMPLICACIONES NUTRICIONALES
(Efecto en la alimentación)

RECOMENDACIONES DIETÉTICAS
(Ajustes sugeridos)

Analiza ahora el archivo adjunto:`;
...
async function analyzeLabWithGemini(
  ...
  if (match) {
    ...
    // Es una URL de Supabase u otra URL externa, la descargamos y convertimos a base64
    try {
      const response = await fetch(fileUrl);
      if (!response.ok) throw new Error('No se pudo descargar el archivo para análisis.');
      ...
      console.error('Error downloading file for AI analysis:', err);
      throw new Error('No se pudo procesar el archivo para la IA. Intenta descargarlo y subirlo de nuevo o verifica tu conexión.');
    }
  }
  ...
  try {
    const response = await aiService.invokeGemini(parts, 'labs');
    const geminiData = response.output;
