...
// Secrets: supabase secrets set RECURRENTE_API_SECRET_KEY=sk_xxx  (same secret used by
//          update-payment-method and recurrente-webhook for privileged /api/subscriptions/{id} calls)
...
serve(async (req: Request) => {
  ...
  if (sub.recurrente_subscription_id) {
    ...
    const recurrenteRes = await fetch(
      ...
      {
        method:  'DELETE',
        headers: { 'X-SECRET-KEY': RECURRENTE_API_SECRET_KEY },
      },
    );
  ...
  // and skip the immediate downgrade. authStore calls expire-subscription once
  // current_period_end passes, which downgrades and sends the Plan Básico email.
  ...
  if (updateError) {
    console.error('Failed to update subscription to cancelled_pending:', updateError);
    return json({ ok: false, message: 'Error al guardar la cancelación. Intenta de nuevo.' }, 500);
  }
